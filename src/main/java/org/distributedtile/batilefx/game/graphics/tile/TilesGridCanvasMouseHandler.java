...
import org.distributedtile.batilefx.game.graphics.interfaces.ITilesGridRegion;
import org.distributedtile.batilefx.game.map.interfaces.IMapService;
...
public class TilesGridCanvasMouseHandler implements EventHandler<MouseEvent>{
	...
	private boolean onCanvas,pressedFlag;
	...
	public void handle(MouseEvent event) {
		...
		if(event.getEventType().equals(MouseEvent.MOUSE_MOVED)) {
			...
			pressedFlag = false;
			if( !onCanvas || !(pressedRow == currentRow && pressedCol == currentCol) ) {
				// 产生两个事件,一个事件是离开旧瓷砖，一个进入新瓷砖
				exitedEvent = 
						new TileMouseEvent(pressedRow, pressedCol, evenRegion, event, TileMouseEventType.MOUSE_EXITED);
				if( onCanvas ) {
					enteredEvent =
						new TileMouseEvent(currentRow, currentCol, evenRegion, event, TileMouseEventType.MOUSE_ENTERED);
				}
		...
		// 记录刷新
		if(pressedFlag) {
			recordRow = this.pressedRow;
			recordCol= this.pressedCol;
		} else {
			recordRow = currentRow;
			recordCol = currentCol;
		}
